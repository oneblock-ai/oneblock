...
// ModelTemplateVersionStatusHandler is executed for every added or modified ModelTemplateVersion. Should return the new status to be updated
...
// ModelTemplateVersionGeneratingHandler is the top-level handler that is executed for every ModelTemplateVersion event. It extends ModelTemplateVersionStatusHandler by a returning a slice of child objects to be passed to apply.Apply
...
// RegisterModelTemplateVersionStatusHandler configures a ModelTemplateVersionController to execute a ModelTemplateVersionStatusHandler for every events observed.
...
// RegisterModelTemplateVersionGeneratingHandler configures a ModelTemplateVersionController to execute a ModelTemplateVersionGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured ModelTemplateVersionGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *modelTemplateVersionGeneratingHandler) isNewResourceVersion(obj *v1.ModelTemplateVersion) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *modelTemplateVersionGeneratingHandler) storeResourceVersion(obj *v1.ModelTemplateVersion) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
